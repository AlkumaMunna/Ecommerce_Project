...
def add_to_cart(request, pk):
    ...
    order_qs = Order.objects.filter(user=request.user, ordered=False) # check if user has any incomplete order
    if order_qs.exists():
        order = order_qs[0] #incomplete order
...
def cart_view(request):
    ...
    if carts.exists() and orders.exists():
        order = orders[0] # converts the orderlist from tuple to object
...
def remove_from_cart(request, pk):
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists:
            ...
            order.orderitems.remove(order_item) # orderitem will delete from order table 
